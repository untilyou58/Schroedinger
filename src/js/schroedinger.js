...
export function Schroedinger(settings, p5) {
  ...
  this.plotPot = new Array(p5?.width);
  this.plotX = new Array(p5?.width);
  this.plotP = new Array(p5?.width);
  ...
  this.computePsiAndV = function (startIndex, sign) {
    ...
    for (let i = startIndex; i > 0 && i < this.size - 1; i += sign) {
      ...
      if (this.gaussian) {
        let velocity = new Complex(
          Math.cos(this.velocity * this.xStep * i),
          Math.sin(this.velocity * this.xStep * i)
        );
      ...
      let increment =
        sign *
        Math.sqrt(2 * Math.abs(this.energy - this.potEnergy[i])) *
        this.xStep;
  ...
  this.updateMomenta = function () {
    ...
    for (let i = 0; i < this.size / 2; i++) {
      [this.psiP[i], this.psiP[i + this.size / 2]] = [
        this.psiP[i + this.size / 2],
        this.psiP[i],
      ];
    }
  ...
  this.computeTridiagonalMatrix = function () {
    let imaginary = new Complex(
      0,
      (4 * this.xStep * this.xStep) / this.timeStep
    );
    let scalar = this.xStep * this.xStep * 2;

    for (let i = 1; i < this.size - 1; i++) {
      this.diagT[i] = new Complex().add(
        imaginary,
        scalar * this.potEnergy[i],
        2
      );
      this.auxA[i] =
        i > 1 ? new Complex(-1).mul(this.auxA[i - 1]) : new Complex();
      this.auxA[i]
        .sub(imaginary)
        .add(scalar * this.potEnergy[i], 2)
        .invert();
    }
  };
  ...
  this.updateStatistics = function () {
    ...
    this.statistics.rmsX = Math.sqrt(
      this.statistics.rmsX / this.statistics.normX
    );
    this.statistics.rmsP = Math.sqrt(
      this.statistics.rmsP / this.statistics.normP
    );
  ...
  this.propagate = function () {
    ...
    for (let step = 0; step < this.stepsPerFrame; step++) {
      for (let i = 1; i < this.size - 1; i++) {
        auxB[i] =
          i > 1
            ? new Complex(auxB[i - 1]).mul(this.auxA[i - 1])
            : new Complex();
        auxB[i].add(
          new Complex(this.diagT[i])
            .mul(this.psiX[i])
            .sub(this.psiX[i - 1], this.psiX[i + 1])
        );
      }
  ...
  this.saveAverageDensity = function () {
    let densityData = new p5js.Table();
    ...
    for (let i = 0; i < this.size; i++) {
      ...
      newRow.setString(
        "Density rhoX",
        (this.rhoX[i] / intRhoX).toExponential(8)
      );
      newRow.setString(
        "Momentum p",
        ((i - this.size / 2) * this.pStep).toExponential(8)
      );
      newRow.setString(
        "Density rhoP",
        (this.rhoP[i] / intRhoP).toExponential(8)
      );
    }

    p5?.saveTable(densityData, this.dataFile + "Density.csv");
